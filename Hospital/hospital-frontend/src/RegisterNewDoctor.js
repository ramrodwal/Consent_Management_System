...
function RegisterNewDoctor() {
  ...
  const handleSubmit = async (event) => {
    ...
    const practitionerDetails = {
      ...
      zipcode: zipcode, practitionerAadhar: practitionerAadhar, medicalLicenseId: medicalLicenseId, specialisation: specialisation, qualification: qualification,
      password: password, confirmPassword: confirmPassword, centralHospital: { hospitalId: hospitalId.centralHospital.hospitalId }
    };
    if (isValidEmail(email) && isValidPhoneNumber(number) && isValidAadharNumber(practitionerAadhar)
      ...
      await axios.post('http://localhost:9099/api/auth/register', practitionerDetails)
